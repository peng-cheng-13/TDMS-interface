...
jobject Env::getFs(){
  ...
  jobject obj;
  try{
    cls = m_env->FindClass("InitTDMS");
    mid = m_env->GetStaticMethodID(cls, "gethost", "()Lalluxio/client/file/FileSystem;");
    obj = m_env->CallObjectMethod(cls, mid);
  } catch (const NativeException& exce) {
    throw NewObjectException("InitTDMS", exce.detail());
  }
  return obj;
}

jobject  Env::getLocationPolicy(char* myLocationPolicy, char* targethost){
  jclass cls;
  jmethodID mid;
  jobject obj;
  jstring jmyLocationPolicy = m_env->NewStringUTF(myLocationPolicy);
  try{
    cls = m_env->FindClass("InitTDMS");
    mid = m_env->GetStaticMethodID(cls, "getLocationPolicy", "(Ljava/lang/String;Ljava/lang/String;)Lalluxio/client/file/policy/FileWriteLocationPolicy;");
    printf("Find getLocationPolicy successed \n");
    if(targethost != NULL){
        jstring jtargethost =  m_env->NewStringUTF(targethost);
	obj = m_env->CallObjectMethod(cls, mid,jmyLocationPolicy, jtargethost);
    }else{
	//jstring jtargethost =  m_env->NewStringUTF("tt");
	obj = m_env->CallObjectMethod(cls, mid,jmyLocationPolicy,NULL);
    }
  }catch (const NativeException& exce) {
    throw NewObjectException("InitTDMS", exce.detail());
  }
...
JNIEnv* JNIHelper::getEnv()
{
  ...
  if (vms == 0) {
    ...
    options[1].optionString = "-Xmx4572m";
    options[2].optionString = "-Xms4096m";
